    ...
    public partial class Borrower : Form
    {
        ...
        private void btn_Update_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (DS.Rows.Count == 0)
                {
                    MessageBox.Show("No record found regarding this Borrower.");
                }
                else
                {
                    ...
                    using (SqlConnection conn = new SqlConnection(@"Data Source = (LocalDB)\MSSQLLocalDB; Initial Catalog = LibDB; Integrated Security = True"))
                    {
                        using (SqlCommand cmd = new SqlCommand("spUpdateBorrowers", conn))
                        {
                            cmd.CommandType = CommandType.StoredProcedure;
        ...
        private void btn_Delete_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (DS.Rows.Count == 0)
                {
                    MessageBox.Show("This Borrower does not exist.");
                }
                else
                {
                    using (SqlConnection conn = new SqlConnection(@"Data Source = (LocalDB)\MSSQLLocalDB; Initial Catalog = LibDB; Integrated Security = True"))
                    {
                        using (SqlCommand cmd = new SqlCommand("spDeleteBorrowers", conn))
                        {
                            cmd.CommandType = CommandType.StoredProcedure;

                            cmd.Parameters.Add("@borrowerId", SqlDbType.VarChar).Value = txt_BorrowerID.Text;
        ...
        {
            string query = "Select * from Borrower where Name ='" + txt_Search.Text + "'";
